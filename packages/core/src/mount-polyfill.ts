/**
 * @ios-web-bluetooth/core — polyfill mount logic.
 *
 * This is the single source of truth for installing the Web Bluetooth polyfill
 * on `navigator.bluetooth`. Both the bare entry (`src/index.ts`) and the
 * side-effect entry (`src/auto.ts`) import `applyPolyfill` from here and invoke
 * it at module load, so `import '@ios-web-bluetooth/core'` and
 * `import '@ios-web-bluetooth/core/auto'` are behaviourally identical.
 *
 * This module must remain free of top-level side effects — it only defines
 * helpers. Callers are responsible for invoking `applyPolyfill()`.
 *
...
import { emitSdkLoadedOrigin } from './attribution-hook';
...
/**
 * Mount the Web Bluetooth polyfill. Idempotent via `!bluetoothNavigator.bluetooth`
 * guards — safe to call from multiple entry points without clobbering the
 * native or already-installed surface.
 */
export function applyPolyfill(): void {
  if (typeof navigator === 'undefined') return;
  try {
    mountBluetoothSurface();
  } finally {
    // AIDEV-NOTE: Attribution round-trip (Wave VIII gap). Runs on every
    // supported mount path — native, safari-extension, and the lazy
    // unsupported-fallback proxy — so every distribution channel that
    // stamps a `data-webble-attr` script sees the beacon regardless of
    // which browser the consumer lands on. The hook schedules itself
    // via requestIdleCallback / setTimeout; it is non-blocking and
    // cannot throw out of this frame.
    emitSdkLoadedOrigin();
  }
}

function mountBluetoothSurface(): void {
